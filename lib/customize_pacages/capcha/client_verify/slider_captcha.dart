...
class SliderCaptcha extends StatefulWidget {
  const SliderCaptcha({
    ...
    this.colorCaptChar = Colors.white,
    this.controller,
    this.borderImager = 0,
...
class _SliderCaptchaState extends State<SliderCaptcha>
  ...
  String img = 'assets/capcha-images/${Random().nextInt(50) + 1}.png';
  ...
  Future<void> checkAnswer() async {
    if (isLock) return;
    isLock = true;
    print("offset => ${_offsetMove}");
    print("answerx => ${answerX}");
    print("answery => ${answerY}");
    print("threshold => ${widget.threshold}");
    print("Cond 1 => ${_offsetMove < answerX + widget.threshold}");
    print("Cond 2 => ${_offsetMove > answerX - widget.threshold}");
...
class _RenderTestSliderCaptChar extends RenderProxyBox {
  ...
  Offset? create() {
    ...
    createX = sizeCaptChar + size.width - randomInRange(3,8) * sizeCaptChar;

    createY = sizeCaptChar + size.width - randomInRange(3,8) * sizeCaptChar;
